class DropBoxController {
  ...
  getFirebaseRef(path) {
    ...
    return firebase.database().ref(path);//fire nao esquecer 
  }
  ...
  renderNav(){
    ...
    for (let i = 0; i < this.currentFolder.length; i++){
      ...
      if ((i + 1)=== this.currentFolder.length){
        ...
        span.className = '<span class="breadcrumb-segment__wrapper">'
        span.innerHTML = `
  ...
  initEventsLi(li) {
    ...
    li.addEventListener('dblclick', e=>{
      ...
      switch(file.type){
        ...
        default:
          window.open('/file?path=' + file.path);//colocar o file
      }
